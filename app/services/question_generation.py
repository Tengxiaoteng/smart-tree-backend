...
class QuestionPlanItem(BaseModel):
    ...
    target_goal_id: str | None = Field(None, alias="targetGoalId")  # 关联的学习目标ID
    target_goal_name: str | None = Field(None, alias="targetGoalName")  # 关联的学习目标名称
...
请规划题目分布，确保考察点覆盖各个来源的核心内容。"""
        ...
        # 获取可用的学习目标
        available_goals = request.learning_goals if request.learning_goals else []
        
        result_plans = []
        for i, p in enumerate(plans[:request.count]):
            # 轮流分配学习目标
            target_goal = available_goals[i % len(available_goals)] if available_goals else None
            result_plans.append(QuestionPlanItem(
                index=p.get("index", i),
                ...
                target_goal_name=target_goal.goal if target_goal else None,
            ))
        return result_plans
...
3. 提供 variationHints，便于生成变式题"""
    ...
    def _build_question_from_response(
        ...
        return QuestionSchema(
            ...
            target_goal_id=plan.target_goal_id,
            target_goal_name=plan.target_goal_name or plan.focus_point,
...
3. 提供 variationHints，便于生成变式题"""
    ...
    for i, q in enumerate(parsed[:request.count]):
        ...
        questions.append(QuestionSchema(
            ...
            target_goal_id=q.get("targetGoalId"),
...
async def generate_questions_batch(
    ...
    if resolved.mode == "system":
        # 官方 API: 使用两阶段并行策略
        # 积分扣除
        request_id = f"question_generate:{uuid.uuid4()}"
        fixed_points = calculate_fixed_points(FeatureType.QUESTION_GENERATE)
        ...
        try:
            credits_service.reserve_points(
                ...
                points=fixed_points,
                meta={"feature": "question_generate", "count": request.count},
            )
        except Exception as e:
            return QuestionGenerationResult(
                success=False,
        ...
        try:
            ...
            credits_service.finalize_reservation(
                ...
                meta={"feature": "question_generate", "success": True, "count": len(questions)},
            )
            return QuestionGenerationResult(
                success=True,
                questions=questions,
            ...
            credits_service.finalize_reservation(
                ...
                reserved_points=fixed_points,
                actual_points=0,
                meta={"feature": "question_generate", "error": str(e)},
            )
            return QuestionGenerationResult(
                success=False,
                error=str(e),
...
async def generate_questions_batch_stream(
    ...
    if resolved.mode == "system":
        # 官方 API: 使用流式并行策略
        # 积分扣除
        request_id = f"question_generate_stream:{uuid.uuid4()}"
        fixed_points = calculate_fixed_points(FeatureType.QUESTION_GENERATE)
        ...
        try:
            credits_service.reserve_points(
                ...
                request_id=request_id,
                points=fixed_points,
                meta={"feature": "question_generate_stream", "count": request.count},
            )
        except Exception as e:
            yield QuestionGenerationResult(
                success=False,
        ...
        try:
            ...
            credits_service.finalize_reservation(
                ...
                reserved_points=fixed_points,
                actual_points=fixed_points,
                meta={"feature": "question_generate_stream", "success": True, "count": len(questions)},
            )
            yield QuestionGenerationResult(
                success=True,
                questions=questions,
            ...
            credits_service.finalize_reservation(
                ...
                reserved_points=fixed_points,
                actual_points=0,
                meta={"feature": "question_generate_stream", "error": str(e)},
            )
            yield QuestionGenerationResult(
                success=False,
                error=str(e),
