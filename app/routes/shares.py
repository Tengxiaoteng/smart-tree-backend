...
from app.core.file_refs import extract_file_id_from_proxy_url, extract_proxy_file_ids_from_text
from app.core.security import get_current_user, get_current_user_optional
from app.core.user_files import ensure_user_files_copied, replace_proxy_file_ids_in_json, replace_proxy_file_ids_in_text
...
def _remap_proxy_url(url: str | None, file_id_map: dict[str, str]) -> str | None:
    if not url or not file_id_map:
        return url
    file_id = extract_file_id_from_proxy_url(url)
    if not file_id:
        return url
    new_id = file_id_map.get(file_id)
    if not new_id:
        return url
...
async def copy_shared_tree(
    ...
    for node_data in snapshot.get("nodes", {}).values():
        source_file_ids.update(extract_proxy_file_ids_from_text(node_data.get("description")))
        for field in ("learningObjectives", "keyConcepts", "questionPatterns", "commonMistakes", "aiInferredGoals"):
            value = node_data.get(field)
            if value is not None:
                source_file_ids.update(extract_proxy_file_ids_from_text(str(value)))

    for material_data in snapshot.get("materials", {}).values():
        file_id = extract_file_id_from_proxy_url(material_data.get("url"))
        if file_id:
            source_file_ids.add(file_id)
        for field in ("content", "organizedContent", "aiSummary"):
            source_file_ids.update(extract_proxy_file_ids_from_text(material_data.get(field)))
        for field in ("structuredContent", "extractedConcepts"):
    ...
    for old_node_id, node_data in snapshot["nodes"].items():
        new_node = KnowledgeNode(
            ...
            description=replace_proxy_file_ids_in_text(node_data.get("description"), file_id_map),
            parentId=None,  # 先设为 None，稍后更新
            children=[node_id_map.get(c) for c in (node_data.get("children") or []) if c in node_id_map],
            learningObjectives=replace_proxy_file_ids_in_json(node_data.get("learningObjectives"), file_id_map),
            keyConcepts=replace_proxy_file_ids_in_json(node_data.get("keyConcepts"), file_id_map),
            ...
            questionPatterns=replace_proxy_file_ids_in_json(node_data.get("questionPatterns"), file_id_map),
            commonMistakes=replace_proxy_file_ids_in_json(node_data.get("commonMistakes"), file_id_map),
            aiInferredGoals=replace_proxy_file_ids_in_json(node_data.get("aiInferredGoals"), file_id_map),
    ...
    # 更新节点的 parentId（所有节点已创建，现在可以安全设置外键）
    for old_node_id, node_data in snapshot["nodes"].items():
        old_parent_id = node_data.get("parentId")
    ...
    for old_material_id, material_data in snapshot["materials"].items():
        ...
        new_material = Material(
            ...
            content=replace_proxy_file_ids_in_text(material_data.get("content"), file_id_map),
            url=_remap_proxy_url(material_data.get("url"), file_id_map),
            ...
            organizedContent=replace_proxy_file_ids_in_text(material_data.get("organizedContent"), file_id_map),
            aiSummary=replace_proxy_file_ids_in_text(material_data.get("aiSummary"), file_id_map),
            extractedConcepts=replace_proxy_file_ids_in_json(material_data.get("extractedConcepts"), file_id_map),
            isOrganized=material_data.get("isOrganized", False),
            structuredContent=replace_proxy_file_ids_in_json(material_data.get("structuredContent"), file_id_map),
...
async def sync_subscription(
    ...
    for orig_node_id in accepted_node_ids:
        node_data = new_snapshot.get("nodes", {}).get(orig_node_id)
        if not node_data:
            continue
        source_file_ids.update(extract_proxy_file_ids_from_text(node_data.get("description")))
        for field in ("learningObjectives", "keyConcepts", "questionPatterns", "commonMistakes", "aiInferredGoals"):
    ...
    for orig_mat_id in accepted_material_ids:
        mat_data = new_snapshot.get("materials", {}).get(orig_mat_id)
        if not mat_data:
            continue
        file_id = extract_file_id_from_proxy_url(mat_data.get("url"))
        if file_id:
            source_file_ids.add(file_id)
        for field in ("content", "organizedContent", "aiSummary"):
            source_file_ids.update(extract_proxy_file_ids_from_text(mat_data.get(field)))
        for field in ("structuredContent", "extractedConcepts"):
    ...
    for orig_node_id in accepted_node_ids:
        ...
        if local_node_id:
            ...
            if node:
                ...
                node.description = replace_proxy_file_ids_in_text(node_data.get("description"), file_id_map)
                node.learningObjectives = replace_proxy_file_ids_in_json(node_data.get("learningObjectives"), file_id_map)
                node.keyConcepts = replace_proxy_file_ids_in_json(node_data.get("keyConcepts"), file_id_map)
                ...
                node.questionPatterns = replace_proxy_file_ids_in_json(node_data.get("questionPatterns"), file_id_map)
                node.commonMistakes = replace_proxy_file_ids_in_json(node_data.get("commonMistakes"), file_id_map)
                node.aiInferredGoals = replace_proxy_file_ids_in_json(node_data.get("aiInferredGoals"), file_id_map)
                node.prerequisites = [node_mapping.get(p) for p in (node_data.get("prerequisites") or []) if p in node_mapping]
                nodes_updated += 1
        else:
            # 添加新节点
            new_node_id = str(uuid.uuid4())
            ...
            new_node = KnowledgeNode(
                ...
                commonMistakes=replace_proxy_file_ids_in_json(node_data.get("commonMistakes"), file_id_map),
                aiInferredGoals=replace_proxy_file_ids_in_json(node_data.get("aiInferredGoals"), file_id_map),
                prerequisites=[node_mapping.get(p) for p in (node_data.get("prerequisites") or []) if p in node_mapping],
                materialIds=[material_mapping.get(m) for m in (node_data.get("materialIds") or []) if m in material_mapping],
                source="synced",
            )
            db.add(new_node)
            nodes_updated += 1
    ...
    for orig_mat_id in accepted_material_ids:
        ...
        if local_mat_id:
            ...
            if material:
                ...
                material.type = mat_data.get("type", material.type)
                material.content = replace_proxy_file_ids_in_text(mat_data.get("content"), file_id_map)
                material.url = _remap_proxy_url(mat_data.get("url"), file_id_map)
                material.fileSize = mat_data.get("fileSize", material.fileSize)
                material.tags = mat_data.get("tags", material.tags)
                material.organizedContent = replace_proxy_file_ids_in_text(mat_data.get("organizedContent"), file_id_map)
                material.aiSummary = replace_proxy_file_ids_in_text(mat_data.get("aiSummary"), file_id_map)
                material.extractedConcepts = replace_proxy_file_ids_in_json(mat_data.get("extractedConcepts"), file_id_map)
                material.structuredContent = replace_proxy_file_ids_in_json(mat_data.get("structuredContent"), file_id_map)
                material.isOrganized = mat_data.get("isOrganized", material.isOrganized)
                material.isStructured = mat_data.get("isStructured", material.isStructured)
                # 更新 nodeIds 映射
            ...
            new_material = Material(
                ...
                content=replace_proxy_file_ids_in_text(mat_data.get("content"), file_id_map),
                url=_remap_proxy_url(mat_data.get("url"), file_id_map),
                fileSize=mat_data.get("fileSize"),
                tags=mat_data.get("tags"),
                organizedContent=replace_proxy_file_ids_in_text(mat_data.get("organizedContent"), file_id_map),
                aiSummary=replace_proxy_file_ids_in_text(mat_data.get("aiSummary"), file_id_map),
                extractedConcepts=replace_proxy_file_ids_in_json(mat_data.get("extractedConcepts"), file_id_map),
                structuredContent=replace_proxy_file_ids_in_json(mat_data.get("structuredContent"), file_id_map),
                isOrganized=mat_data.get("isOrganized", False),
                isStructured=mat_data.get("isStructured", False),
    ...
    for orig_node_id in accepted_node_ids:
        node_data = new_snapshot.get("nodes", {}).get(orig_node_id)
        local_node_id = node_mapping.get(orig_node_id)
        if not node_data or not local_node_id:
            continue
        node = db.query(KnowledgeNode).filter(KnowledgeNode.id == local_node_id).first()
        if not node:
            continue
        node.materialIds = [material_mapping.get(m) for m in (node_data.get("materialIds") or []) if m in material_mapping]
        node.prerequisites = [node_mapping.get(p) for p in (node_data.get("prerequisites") or []) if p in node_mapping]
